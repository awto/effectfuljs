...
function d_1(ctx, d_v) {
  ...
  d_v.j = 0;
  a = d_v.i++;
  return M.j(ctx, M.yld(a), d_2, d_v);
}

function d_2(ctx, d_v) {
  var a;
  a = d_v.j++;
  return M.j(ctx, M.yld(a), d_3, d_v);
}
...
function d_3(ctx, d_v) {
  ...
  b = d_v.i += 2;
  a = some(b, d_v.j);
  return M.jB(ctx, M.yldStar(a + d_v.j), d_4, d_v);
}

function d_4(ctx, a, d_v) {
  return M.jB(ctx, M.yld(a), d_5, d_v);
}
