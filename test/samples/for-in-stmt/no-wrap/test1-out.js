...
function g() {
  var loop;
  var i;
  return eff('a').mbind(() => eff(2)).mbind(a => {
    var b = M.forInIterator(a);
    loop = b;
    return M.block(label => M.set({
      loop: loop
      ...
      if (d) return M.block($continue => {
        ...
        return M.set({
          ...
          if (e) return $continue();else return M.pure();
        }).mbind(() => effB(i)).mbind(f => {
          if (f) return label();else return M.pure();
        }).mbind(() => eff('c'));
      });else return M.set({
        loop: loop
      }).mbind(() => label());
          ...
          return M.block(label => M.modify(s => ({
            ...
            if (f) return M.block($continue => {
              ...
              return M.modify(s => ({
                loop: s.loop,
                loop1: loop1
              })).mbind(() => eff('b')).mbind(() => eff(i + j).mbind(g => {
                if (g) return lab$continue();else return M.pure();
              }).mbind(() => effB(i + j))).mbind(k => {
                if (k) return lab();else return M.pure();
              }).mbind(() => eff(i)).mbind(m => {
                if (m) return $continue();else return M.pure();
              }).mbind(() => effB(i)).mbind(n => {
                if (n) return label();else return M.pure();
              }).mbind(() => eff('c'));
