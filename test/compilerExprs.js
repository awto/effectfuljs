...
describe('expressions translator', function() {
  ...
  it('should transform nested effectful expressions to bind', function() {
    exprEqual(
      ...
      function () {
        return M(effb(2)).mbind(a => eff(a)).mapply(b => {
          b + 2;
        });
      });
  });
    ...
    exprEqual(
      ...
      function () {
        return M(eff(1)).mbind(a => {
          var b = console.log(a, 2);
          return M(eff(2)).mapply(c => {
            b + c * 2;
          });
        });
      })
    ...
    exprEqual(
      ...
      function () {
        return M(eff(1))
          .mbind(
            a => M(eff(2)).mbind(
              b => M(eff(3)).mbind(c => b(c))).mbind(d => a(d)));
      });
    exprEqual(
      function() {
        eff(1)(eff(2), eff(3), 4);
  ...
  it('should combine bind in mapplys accordingly', function() {
    exprEqual(
      ...
      function () {
        return M(effb(2)).mbind(a => eff(a)).mbind(b => M(effc(3)).mapply(c => {
          5 + b + c;
        }));
      });
    exprEqual(
    ...
    exprEqual(
      ...
      function () {
        return M(effc(3)).mbind(a => M(effb(2)).mbind(b => eff(b + 4)).mapply(c => {
          5 + a + c;
        }));
      });
    exprEqual(
