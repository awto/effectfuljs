...
module.exports = process.env.EFFECTFUL_TRANSFORM_V2
  ...
  : require("@effectful/core").babelPlugin(
      (opts, { Tag, Kit, Transform: T, Policy: P, presets, Block }) => {
        ...
        return {
          options: {
            ...
            wrapPropAccess: !!opts.timeTravel
              ? {
                  set: { name: "set", local: "lset", global: "gset" },
                  delete: { name: "del", local: "ldel", global: "gdel" }
                }
              : null,
            ...opts,
            moduleAliases,
          ...
          main(input) {
            ...
            if (VERBOSE)
              console.log(
                `DEBUGGER: transforming:"${(opts.file && opts.file.filename) ||
                  "local"}", blackbox:${!!opts.blackbox}, timeTravel:${!!opts.timeTravel}`
              );
        ...
        function inline(si) {
          const scope = Kit.sysId("scope");
          const scopeA = Kit.sysId("scopeA");
          const s = Kit.auto(si);
          ...
          function* _inline() {
            for (const i of s) {
              if (i.enter && i.type === Block.letStmt) {
                const op = i.value.opSym;
                if ((op === scope && !opts.blackbox) || op === scopeA) {
                  yield* s.toks(
                    Tag.push,
                    `*$1.goto = $2; try { return ${
                      opts.blackbox || opts.hot === false
                        ? "$4"
                        : "$1.meta.handler"
                    }($1,void 0); } catch(e) { return $3($1,e); } `,
                    root.contextSym,
                    i.value.goto.declSym,
                    handleSym,
        ...
        /** saving function's reference in call so we know the function is called in managed stack */ function* safeCalls(
          si
        ) {
          const s = Kit.auto(si);
              ...
              if (
                ...
                if (
                  ...
                  if (
                    ...
                    s.take();
                    if (opts.expNoCallWraps) {
                      const calleeExpr = [callee, ...s.sub(), s.close(callee)];
                      yield* s.template(
                        i.pos,
                        "=$I.call = $E, $E",
                        contextOpSym
                      );
                      yield* Kit.reposOne(Kit.clone(calleeExpr), Tag.right);
                      yield* s.refocus();
