    ...
    function() {
      ...
      for(let j = 0; j < res.length; ++j) {
        ...
        if (e.done || res[j] !== e.value) {
          throw new Error(`wrong output: for ${name} on - ${j} ${
          e.done} || ${res[j]} !== ${e.value}`)
        }
      }
      const last = i.next(-1)
      if (!last.done)
        throw new Error("wrong result:" + name)
    } : function() {
      for(const i of iter()){}
    }
}
...
function benchmark(name) {
  ...
  for(const j in impls) {
    suite = suite.add(`${name}:${j}`,
                      run(`${name}:${j}`,impls[j][name],result))
  }
  suite = suite.add(`${name}:native`,
