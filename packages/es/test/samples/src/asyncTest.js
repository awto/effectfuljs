...
let send, sendErr, evaluate;
{
  ...
  function wrapCancel(data, p) {
    p[ES.cancelSymbol] = () => (
      (data.canceled = true), cancelJobs.push(data.arg)
    );
    return p;
  }
  ...
  function pack(arg, timeout, result) {
    if (cont) cont();
    if (result === copy) result = arg;
    if (arg === stop) return null;
    const j = { arg, timeout, result, at: timeout + t };
    scheduleJobs.push(j);
    const order = doneJobs.findIndex(k => k.at > j.at);
    if (order < 0) doneJobs.push(j);
    else doneJobs.splice(order, 0, j);
    return j;
  }
...
describe("async `try/catch`", function () {
  it("should conform to ES", async function () {
    assert.deepEqual(
      ...
      ["> a_1@0", "< a_1@0", "> E:A@0", "< E:A@0", "> a-after@0", "< a-after@0"]
    );
    assert.deepEqual(
      await evaluate(
        (async function ex2() {
    ...
    assert.deepEqual(
      ...
      ["> A@0", "< A@0", "> E:A/?@0", "< E:A/?@0", "> a-after@0", "< a-after@0"]
    );
    assert.deepEqual(
      await evaluate(
        (async function ex4() {
  ...
  context("block of independent computations", function () {
    ...
    context("insite `try/catch`", function () {
      it("should cancel running threads on throw", async function () {
        ...
        assert.deepEqual(await evaluate(a()), [
          "> a,error,d,g@0",
          "< a@10",
          "> b@10",
          "< d@12",
    ...
    context("with a few independent computations body", function () {
      it("should run the body an each iteration in parallel", async function () {
        assert.deepEqual(
          ...
          [
            "> a_1,b_1,a_2,b_2,a_3,b_3,a_4,b_4,a_5,b_5@0",
            "< a_1@0",
            "< b_1@0",
            "< a_2@0",
            "< b_2@0",
        ...
        it("should run each iteration in parallel but the body sequentially", async function () {
          assert.deepEqual(
            ...
            [
              ...
              "< a_4@0",
              "< a_5@0",
              "> b_0_1,b_0_2,b_0_3,b_0_4,b_0_5@0",
              "< b_0_1@0",
              "< b_0_2@0",
              "< b_0_3@0",
              "< b_0_4@0",
  ...
  context("when next parallel changes variables values", function () {
    it("should use correct values for variables", async function* a() {
      ...
      assert.deepEqual(
        await evaluate(
          (async function a11() {
            ...
            for (let j = 0; j < 3; j++) {
              k = `a-${j}`;
              await send(
                `${k}*-${j}-${await send(`${k}_${j}`, 20 * (2 - j) + 10)}`,
                20 * j
              );
              k = `b-${j}`;
              await send(
                `${k}*-${j}-${await send(`${k}_${j}`, 20 * j + 5)}`,
                20 * (2 - j)
              );
            }
        ...
        [
          "> a-0_0,b-0_0,a-1_1,b-1_1,a-2_2,b-2_2@0",
          "< b-0_0@5",
          "> b-0*-0-5@5",
          "< a-2_2@10",
          "> a-2*-2-10@10",
      ...
      assert.deepEqual(
        ...
        [
          "> k-a-0,k-b-0,k-a-1,k-b-1,k-a-2,k-b-2@0",
          "< k-a-0@0",
          "< k-b-0@0",
          "< k-a-1@0",
          "< k-b-1@0",
  ...
  context("with variable values withing same fork", function () {
    it("should set variable values like in sequential execution", async function () {
      async function a(v1, v2, t1, t2, t3, t4) {
        "par";
        let j = 0;
        if (await send(`sa`, t1, v1)) j = 10;
        if (await send(`sb`, t2, v2)) j = 20;
        await send(`ta:${j}`, t3);
        j++;
        await send(`tb:${j}`, t4);
      }
      assert.deepEqual(await evaluate(a(true, false, 10, 5, 10, 5)), [
        "> sa,sb@0",
        "< sb@5",
        "< sa@10",
        "> ta:10,tb:11@10",
      ...
      assert.deepEqual(await evaluate(a(false, false, 10, 5, 10, 5)), [
        "> sa,sb@0",
        "< sb@5",
        "< sa@10",
        "> ta:0,tb:1@10",
      ...
      assert.deepEqual(await evaluate(a(true, true, 10, 5, 10, 5)), [
        "> sa,sb@0",
        "< sb@5",
        "< sa@10",
        "> ta:20,tb:21@10",
      ...
      assert.deepEqual(await evaluate(a(true, true, 5, 10, 5, 10)), [
        "> sa,sb@0",
        "< sa@5",
        "< sb@10",
        "> ta:20,tb:21@10",
      ...
      assert.deepEqual(await evaluate(b()), [
        "> b:0,d:1@0",
        "< d:1@5",
        "> c:5:1@5",
        "< b:0@10",
      ...
      assert.deepEqual(await evaluate(c()), [
        "> b:0,d:1,f:2@0",
        "< f:2@5",
        "> e:5/2@5",
        "< d:1@10",
      ...
      it("should set variable values like in sequential execution", async function () {
        async function a(v1, v2, t1, t2, t3, t4, t5, t6, t7, t8) {
          ...
          for (const i of [1, 2]) {
            let j = 0;
            {
              if (await send(`sa:${i}`, i === 1 ? t1 : t5, i & v1)) j = 10;
              if (await send(`sb:${i}`, i === 1 ? t2 : t6, i & v2)) j = 20;
        ...
        assert.deepEqual(
          ...
          [
            ...
            "< sa:2@10",
            "< sb:2@10",
            "> a_1:2/20,a_2:2/21@10",
            "< a_1:2/20@20",
            "< a_2:2/21@20",
            "< sa:1@100",
            "< sb:1@100",
            "> a_1:1/10,a_2:1/11@100",
            "< a_1:1/10@200",
            "< a_2:1/11@200"
          ]
        );
        ...
        assert.deepEqual(
          ...
          [
            ...
            "< sb:1@100",
            "< sb:2@100",
            "> a_1:1/20,a_2:1/21,a_1:2/10,a_2:2/11@100",
            "< a_1:1/20@110",
            "< a_1:2/10@110",
            "< a_2:1/21@200",
            "< a_2:2/11@200"
    ...
    context("with nested loops", function () {
      it("should set variable values like in sequential execution", async function () {
        ...
        assert.deepEqual(await evaluate(a(true)), [
