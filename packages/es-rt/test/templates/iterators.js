module.exports = function(Lib) {
  ...
  describe("lean iterator", function() {
    it("has isomorphic conversion to ES iterators", function() {
      Array.from(Kit.eqlGens([1, 2, 3], Lib.iterator([1, 2, 3])));
      Array.from(
        Kit.eqlGens(new Set([10, 20, 30]), Lib.iterator(new Set([10, 20, 30])))
      );
        ...
        return Promise.all([
          Kit.asyncRunAll(
            Kit.eqlAsyncGens(
              ...
              Lib.esAsyncIterator(Lib.iteratorM(Kit.delayIterator([1, 2, 3])))
            )
          ),
          Kit.asyncRunAll(
            Kit.eqlAsyncGens(
              ...
              Lib.esAsyncIterator(
                Lib.iteratorM(Kit.delayIterator(new Set([10, 20, 30])))
              )
            )
          ),
          b.then(() => d).then(() => e),
