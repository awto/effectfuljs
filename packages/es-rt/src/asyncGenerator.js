...
export var AsyncGenerator = function AsyncGenerator() {
  ...
  ctx.$proceed = function() { proceed(ctx) }
  if (!process.env.EJS_NO_CANCEL) {
    ctx.$redirCancel = function() { return redirCancel(ctx) }
  }
  this.$q = []
}
...
if (!process.env.EJS_INLINE) {
  ...
  AGp.chain = function chain(p, step) {
    const ctx = this
    return Promise.resolve(this.$await = p)
      .then(
        function(v) {
          ctx.$await = null
          ctx.$step = step
...
function proceed(ctx) {
  ...
  var i = ctx.$q.shift()
  ctx.$busy = true
  Promise.resolve(runImpl(ctx, i.value))
    .then(function(v) {i.cont(v)}, function(e) {i.errCont(e)})
}
...
AGp.next = function next(value) {
  ...
  if (!process.env.EJS_NO_UNWRAP_ASYNC_ITERATOR) {
    var unwrapped = res.then(function(item) {
      if (!item)
        return
      var value = item.value,
          ures = Promise.resolve(value)
          .then(
            function(v) {
              proceed(ctx)
      ...
      if (process.env.EJS_INLINE || !process.env.EJS_NO_CANCEL) {
        unwrapped[cancelSym] = value && value[cancelSym]
          ? function() { return value[cancelSym]() }
          : nop
      }
      return ures
    }, function(e) {
...
if (!process.env.EJS_INLINE) {
  ...
  AGp.cancel = function() { return redirCancel(this) }
}
