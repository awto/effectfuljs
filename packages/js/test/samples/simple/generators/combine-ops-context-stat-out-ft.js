...
function inner_9(_inner, a) {
  _inner._ex = a;
  return _inner.jump(inner_7, inner_8);
}
...
function f_211(ctx) {
  ctx._loop = ctx.iterator(b);
  return ctx.jump(f_34, f_6);
}

function f_34(ctx) {
  if (!(ctx._loop = ctx._loop.step()).done) {
    ctx._j = ctx._loop.value;
    return ctx.jump(f_41, f_7);
  } else {
    return ctx.pure();
  }
}
