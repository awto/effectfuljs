  ...
  for(let i of s) {
    ...
    if (i.enter) {
      if (i.pos === Tag.top) {
        ...
        if (i.value.errSym)
          txt += `|errSym:[${id(i.value.errSym)}]`
        if (i.value.resSym)
          txt += `|resSym:[${id(i.value.resSym)}]`
        i = D.setComment(i,txt,"hl")
      }
      const v = i.value
      if (v.funcId)
        i = D.setComment(i,`${v.funcId.id}`,"hl")
      if (v.sym) {
      ...
      if (v.declSym != null) {
        ...
        if (v.catchCont) {
          ...
          if (v.catchCont.preCompose)
            hndl += `|ErrPre{${v.catchCont.preCompose.map(
               i => id(i.declSym)+"->"
          ...
          if (v.catchCont.indirJumps && v.catchCont.indirJumps.size)
            hndl += `|ErrIndir{${[...v.catchCont.indirJumps]
              .map(([f,a]) => `${id(a.declSym)}->${id(f.declSym)}`).join()}}`
        ...
        if (v.resultCont) {
          ...
          if (v.resultCont.preCompose)
            hndl += `|ResPre{${v.resultCont.preCompose.map(
               i => id(i.declSym)+"->"
                      +(i.contArg ? id(i.contArg.declSym) : "")).join()}}`
        ...
        const paramStr = all.size
              ? `|Params{${[...all].map(i => id(i) + ":"
                 ...
                 .join()}}` : ""
        i = D.setComment(
          i,`label{${id(v.declSym)}}${paramStr}${hndl}${fin}`,"hl")
        if (i.dstClass)
          i = D.setComment(i,`class{${id(i.dstClass.declSym)}}`)
      }
      if (symName(i.type) === "letStmt" || symName(i.type) === "jump") {
        ...
        if (v.indirJumps && v.indirJumps.size)
          args += `|Indir{${[...v.indirJumps]
            .map(([f,a]) => `${id(a.declSym)}->${id(f.declSym)}`).join()}}`
        const thread = v.threadArgs && v.threadArgs.length ?
