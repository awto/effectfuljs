...
/** 
 * converts ctx symbol to `this` expression inside frames 
 * (handling `contextBy: "this"` option)
 */ 
export function ctxToThis(si) {
  const s = Kit.auto(si)
  const root = s.first.value
  const {contextSym} = root
  if (!contextSym || s.opts.contextBy !== "this")
    return s
  return walk()
  function* walk(insideCtx) {
    for(const i of s.sub()) {
      if (i.enter) {
        switch(i.type) {
        case Tag.Identifier:
          if (i.value.sym === contextSym) {
            if (insideCtx) {
              yield s.tok(i.pos,Tag.ThisExpression)
              s.close(i)
              continue
            }
          }
