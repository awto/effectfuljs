...
async function* fn() {
  ...
  for await (const j of clos2()) yield await new Wait((i += j));
  return await new Wait("result");
}

async function run() {
  for await (const i of fn()) await new Wait(i);
}
...
describe("async generator function", function () {
  ...
  it("should propagate exceptions", function (done) {
    ...
    async function* a3() {
      for await (const i of a2()) yield i;
    }
    async function a4() {
  ...
  describe("current continuation clone", function () {
    it("should be possible to resume", async function () {
      ...
      Object.assign(resume, save);
      assert.deepStrictEqual(await f.next(2), {
        value: "fork:1-2",
        done: true
      });
    });
