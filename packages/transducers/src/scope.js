      ...
      function checkScope(val, syms) {
        ...
        if (report) {
          ...
          for (const i of m.values()) {
            if (i.length > 1) {
              throw s.error(
                `Identifier ${i[0].orig} has already been declared`,
                i[i.length - 1].decl
              );
            }
          }
          ...
          switch (i.type) {
            ...
            case Tag.ArrowFunctionExpression:
              if (!i.leave && s.curLev()) {
                ...
                if (block.type === Tag.BlockStatement) s.take();
                _collectDecls(
                  i.value,
                  block.value,
                  nextSyms,
                ...
                if (block.type === Tag.BlockStatement) s.close(block);
                s.close(i);
              }
              break;
            case Tag.ClassMethod:
            ...
            case Tag.FunctionDeclaration:
              if (!i.leave && s.curLev()) {
                ...
                if (st === "script") sloppy = true;
                else if (st === "module") sloppy = false;
