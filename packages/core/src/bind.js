...
export function optimizePureLets(si) {
  ...
  function* collect() {
    ...
    const lets = new Set(
      cur
        .map(i => i.type === Block.letStmt && i.value.sym)
    ...
    for (const i of cur) {
      ...
      if (i.type === Block.letStmt) {
        if (sym != null) {
          if (lets.has(sym)) candidates.add(sym);
        ...
        case Block.bindPat:
          if (i.enter) {
            ...
            if (seq != null) {
              ...
              if (curlet != null) curlet.push(...rseq);
              else yield* rseq;
              continue;
            }
          }
  ...
  function* walk() {
    for (const i of s) {
      yield i;
      if (i.enter && i.type === Block.frame) yield* collect();
    }
  }
