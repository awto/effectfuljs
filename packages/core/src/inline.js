  ...
  function* _markSimpleRedir() {
    for (const i of s) {
      if (i.enter && i.type === Block.frame) {
        const j = s.cur();
        i.value.simpleRedir =
          (j.type === Block.letStmt && i.value.eff) ||
          j.type === Ctrl.jump ||
          j.type === Block.app;
      }
      yield i;
...
export function promises(si) {
  ...
  if (contextBy === "closure" && defunct) {
    ...
    else localReject = Kit.scope.newSym("$reject");
  }
...
export function promisesClosure(si) {
  ...
  function* _promisesClosure() {
    ...
    if (left)
      yield* s.toks(
        Tag.push,
        `=$I=new Promise(function(rs,rj){$I=rs,$I=rj})`,
        localPromiseSym,
        resolveContSym,
        rejectContSym
      );
...
export function jumpOps(si) {
  ...
  if (!inlineJumps && !inlineScope && !jumpsExit && !promiseJumps) return s;
        ...
        for (const j of s.sub()) {
          if (j.enter) {
            switch (j.type) {
              case Block.app:
                if (
                  (j.value.sym === jumpId && inlineJumps) ||
                  ...
                  yield* bind;
                  if (j.value.hasErrorCont) Kit.skip(s.one());
                  if (j.value.hasResultCont) Kit.skip(s.one());
                  yield* s.sub();
                  yield* lab();
                  s.close(j);
                  continue;
                ...
                if (
                  ...
                  if (promiseRec || promiseJumps) {
                    ...
                    if (j.value.hasBindVal) yield* assignValue(s, ctx);
                    if (j.value.delegateCtx) {
                      yield* s.toks(
                        Tag.push,
                        "=$1.$s.$running = true, $1.$s",
                        { result: true },
                        ctx
                      );
                    } else {
...
export function jsExceptions(si) {
  const s = Kit.auto(si);
  if (!s.opts.inlineJsExceptions || s.opts.defunct) return s;
  const root = s.first.value;
  ...
  function* _jsExceptions() {
    if (skipFirst) yield* s.till(i => i.type === Block.frame && i.leave);
    for (const i of s) {
      yield i;
      if (i.enter && i.type === Block.frame) {
        ...
        if (
          ...
          i.value.last ||
          (!always && (errFrameRedir === goto || i.value.singleJump))
        )
          continue;
...
export function pureOp(si) {
  const s = Kit.auto(si);
  if (!s.opts.inlinePureOp) return s;
...
export function pureOpLeft(si) {
  const s = Kit.auto(si);
  if (s.opts.inlinePureOp !== "promise" || !s.opts.leftChain) return s;
  const root = s.first.value;
  return _pureOpLeft();
  function* _pureOpLeft() {
    ...
    for (const i of s) {
        ...
        switch (i.type) {
          ...
          case Block.app:
            ...
            if (reflected || i.value.reflected)
              ...
              if (i.value.result === false) i.value.reflected = true;
              yield* s.template(
                Tag.push,
                `=$I = void 0,$I($E)`,
                root.promiseLoopSym,
                root.promiseResolveSym
              );
            }
            if (s.curLev()) yield* s.sub();
            yield* s.leave();
      ...
      if (i.leave && i.type === Block.effExpr) reflected = false;
      yield i;
    }
  }
}
...
export function raiseOp(si) {
  const s = Kit.auto(si);
  if (!s.opts.inlineRaiseOp) return s;
        ...
        if (asThrow) {
          ...
          yield* i.value.result === false
            ? s.template(
                ...
                { result: true },
                root.promiseLoopSym,
                root.promiseRejectSym,
                root.localPromiseSym
              )
            : s.template(
                Tag.push,
                `=$I = void 0, $I($E)`,
      ...
      if (
        ...
        for (const j of s.sub()) {
          if (j.enter && j.type === Tag.ThrowStatement) {
            ...
            yield* s.leave();
            s.close(j);
            if (j.pos === Tag.push) Kit.skip(s.sub());
            continue;
          }
          yield j;
        }
  ...
  function* _coerce() {
    for (const i of s) {
      ...
      if (
        ...
        let str = `=$1 && $1.${name}`;
        if (checkIsFunc) str += `&& typeof $1.${name} === "function"`;
        ...
        yield s.enter(
          Tag.alternate,
          Ctrl.jump,
          Object.assign({}, i.value, { bindJump: false, bindName: null })
        );
        yield s.tok(Tag.push, Tag.Identifier, { sym });
        yield* lab();
        s.close(i);
...
export function invertForOf(si) {
  ...
  function inject() {
    ...
    if (!s.opts.inlineContAssign)
      throw s.error(
        "not implemented: `invertForOf` without `inlineContAssign`"
      );
        ...
        if (i.enter) {
          ...
          switch (i.type) {
            ...
            case Ctrl.jump: {
              ...
              yield* s.toks(Tag.push, `=$1.$yld = $2`, ctx, goto.declSym);
              break;
            }
...
function replaceContextByClosure(si) {
  const s = Kit.auto(si);
  if (s.opts.contextBy !== "closure") return s;
  const root = s.first.value;
  const ctx = root.contextSym;
