...
export function* resetOpts(s) {
  const stack = [];
  for (const i of s) {
    if (!i.value.opts) i.value.opts = stack[stack.length - 1].opts;
    if (i.enter) stack.push(i.value);
    if (i.leave) stack.pop(i.value);
    yield i;
  }
}
    ...
    for (const i of s) {
      if (i.enter) {
        ...
        if (
          ...
          i.value.sym &&
          !i.value.sym.declScope &&
          i.value.sym.name === "require"
        ) {
          yield* s.copy(i);
          yield s.peel();
          const j = s.curLev();
          if (j && j.type === Tag.StringLiteral) yield* subst(s.take());
          yield* s.sub();
          yield* s.leave();
          continue;
        }
  ...
  for (const i of s) {
    if (
      ...
      if (obj.type === Tag.Identifier && obj.value.sym === ns) {
        Kit.skip(s.one());
        const prop = s.cur();
        if (prop.type === Tag.Identifier) i.value.nsProp = prop.value.node.name;
      }
    }
  }
}
...
function namespaces(si) {
  ...
  for (const [lib, imps] of imports) {
    for (const { ns, locals } of imps) {
      if (ns) {
        if (libs[lib]) s.opts.importRT = imp = lib;
      ...
      if (i.enter) {
        if (i.type === Match.Placeholder && i.value.v.match) {
          if (i.value.name === "M") {
            ...
            const j = s.cur();
            if (j.type === Tag.Identifier && j.value.sym && j.value.sym === $ns)
              continue;
      ...
      if (i.enter) {
        ...
        if (run == null) {
          throw s.error(
            `profile ${
              i.value.node.name
            } is not defined, available:[${Object.keys(s.opts.profiles)}]`
          );
        }
        const p = yield* run.call(i.value);
        if (p != null) post.push(p);
      }
      continue;
    }
    yield i;
...
export function preprocessPass(s) {
  ...
  function* _preprocessPass(s) {
    if (++num > MAX_PREPROCESS_COUNT) throw new Error("too many transforms");
    s = Kit.share(s);
    for (const i of s) {
      ...
      if (
        ...
        if (prof) {
          ...
          if (q != null) {
            ...
            if (v == null && prof.byId != null) v = prof.byId[q[q.length - 1]];
            if (v == null && prof.byNs != null && q.length > 1)
              v = prof.byNs[q[0]];
            if (v == null && prof.libNs && q.length === 2 && q[0] === $ns.orig)
              v = prof.libNs[q[1]];
            if (v == null) v = prof.all;
            if (v != null) i.value.bind = v;
          }
        }
      }
      yield i;
  ...
  for (const i of s) {
    if (i.type === configDiff) {
      if (i.enter) {
        ...
        else cur.merge = merge(cur.merge || {}, i.value.node);
      }
      continue;
    }
    ...
    if (i.enter) {
      if (cur.level === level) {
        if (cur.merge) {
          i.value.optsDiff = Object.assign(
            i.value.optsDiff || {},
            cur.merge,
            i.value.optsDiff
          );
        ...
        if (cur.assign)
          i.value.optsAssign = Object.assign(
            i.value.optsAssign || {},
            cur.assign,
            i.value.optsAssign
          );
...
export const prepare = Kit.pipe(
  ...
  function(si) {
    ...
    if (configure || preproc) s = Kit.auto(preprocConfig(s));
    if (configure) {
      configure(s);
      s = Kit.auto(propagateOpts(sa));
    }
    if (preproc) return propagateOpts(preproc(Kit.auto(sa)));
    return s;
        ...
        switch (i.type) {
          ...
          case Tag.ExpressionStatement:
            if (s.cur().type === Tag.StringLiteral) {
              if (dir(i, s.cur().value.node.value)) continue;
            }
            break;
          case Tag.Directive:
            if (dir(i, i.value.node.value.value)) continue;
            break;
        }
      }
      yield i;
