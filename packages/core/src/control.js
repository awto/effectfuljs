        ...
        if (i.enter) {
          ...
          switch (i.type) {
            ...
            case Tag.ForOfStatement:
              ...
              for (const j of labs) lmap.set(j, i.value);
              walk([], lmap, nextBrkLab);
              break;
            case Tag.ReturnStatement:
              if (!i.value.last) i.value.block = map.get("#ret");
              break;
                ...
                if (labs.indexOf(ln) === -1)
                  throw new Error(
                    ln !== defaultLab
                      ? `no such label ${ln}`
                      : `unsyntatic ${
                          i.type === Tag.BreakStatement ? "break" : "continue"
                        }`
                  );
              }
              break;
            default:
              if (i.value.func) scope(i);
          }
        }
      }
    }
  ...
  for (const i of sl.sub()) {
    if (i.enter) {
      ...
      if (ref) {
        ...
        else (ref.brkRefs || (ref.brkRefs = [])).push(i.value);
      }
    }
  }
  return sa;
...
export function convolveFrames(sa) {
  ...
  for (const i of s) {
    if (i.enter && i.type === Block.frame) {
      ...
      for (const j of s.sub()) {
        if (j.enter) {
          switch (j.type) {
            case Block.letStmt:
            case jump:
              if (j.value.goto) exits.add(j.value);
          }
          j.value.ref = i.value;
          if (!j.leave) {
              ...
              if (k.enter) {
                switch (k.type) {
                  ...
                  case jump:
                    if (!k.leave) {
                      buf.push(
                        ...(k.value.content = [k, ...s.sub(), s.close(k)])
                      );
                    } else {
                      buf.push(k);
                      k.value.content = [k];
                    }
                    if (k.value.goto) exits.add(k.value);
                    k.value.ref = i.value;
                    continue;
                }
              }
      ...
      if (content.length) content[content.length - 1].fbound = true;
    }
  }
...
export function resetEnters(frames) {
  for (const i of frames) {
    for (const j of i.exits) {
      if (j.gotoDests) {
        for (const k of j.gotoDests) k.enters.add(j);
      } else if (j.goto && j.goto.enters) j.goto.enters.add(j);
    }
  }
}
...
export function flattenGotoDests(goto, res) {
  if (goto.dynamicJump) {
    for (const i of goto.instances) flattenGotoDests(i, res);
  } else {
    res.push(goto);
  }
}
      ...
      if (dst) {
        if (dst.dynamicJump) {
          if (dst.args) {
            ...
            for (const [l, r] of dst.args) args.set(l, r);
          }
          const dests = [];
          ...
          switch (dests.length) {
            case 0:
              ...
              j.gotoDests = [];
              j.goto = ngoto;
              if (ngoto) j.gotoDests.push(ngoto);
              break;
            case 1:
              j.goto = dests[0];
              dst.declSym.bound = false;
