...
export function propagateEff(s) {
  ...
  function skip() {
    for (const i of si.sub()) if (i.enter && i.value.func) func(i);
  }
  function sub(root) {
    let res = false;
    const stack = [];
    ...
    for (const i of si.sub()) {
      if (i.enter) {
        ...
        if (i.value.expr && i.pos !== Tag.top && !i.value.boundary) {
          if (expr(i)) stack[0][0].shallowEff = true;
          continue;
        }
        if (i.value.boundary) {
          const v = i.value;
          ...
          if (v.ctrl != null && v.ctrl.length) {
            v.effDeps = info;
            if (v.ctrl === "#ret" && !coerce) v.bind = true;
          }
          stack.unshift([v, info]);
          if (v.block) {
            const l = v.block.effDeps;
            if (l == null) throw new Error(`no such label ${v.block.ctrl}`);
            if (v.block.eff) propagate(stack);
            else l.push(stack.concat());
          }
        }
      }
      if (i.leave) {
...
export function recalcEff(s) {
  ...
  for (const i of sa) {
    ...
    if (i.enter && i.pos !== Tag.top) {
      ...
      v.shallowEff = null;
      stack.unshift(i);
      if (v.block) v.eff = v.eff || v.block.ctrlEff;
    }
    if (i.leave) {
      stack.shift();
      const j = stack[0];
  ...
  for (const i of s) {
    switch (i.type) {
      ...
      case Tag.ForStatement:
        if (i.enter) stack.unshift(i.level + 1);
