...
function nodeVars(node, position) {
  ...
  if (node.block) {
    for (const i of node.value) {
      ...
      if (frame.patSym) {
        ...
        if (frame.patSym.bound) r.add(frame.patSym);
      }
      w.forEach(setAdd, wv);
    }
    ...
    while (jobs.length) {
      ...
      up: for (const i of job.nonRecExits) {
        if (i._nodeVars !== node) continue;
        for (const j of i.nonRecEnters) if (!j._nodeVarsVisited) continue up;
        jobs.push(i);
      }
      if (job.tryBlock) continue;
      if (job !== first) {
        const inited = job.inited;
        for (const i of job.s) {
          if (!rv.has(i)) inited.add(i);
        }
      ...
      if (i.enter) {
        switch (i.type) {
          ...
          case Tag.Identifier:
            ...
            if (!i.value.sym || !sym.track) break;
            if (i.value.opts.parAllLocals) {
              wv.add(sym);
              rv.add(sym);
            } else {
              if (i.value.rhs) rv.add(sym);
              if (i.value.lhs) _addW(sym, first);
            }
            break;
          case Block.bindPat:
            rv.add(i.value.sym);
  ...
  function _addW(sym, first) {
    if (first && !rv.has(sym)) sv.add(sym);
    else wv.add(sym);
  }
}
...
function edgesByVars(nodes) {
  ...
  for (const dst of nodes) {
    for (const i of dst.rv) {
      ...
      if (srcs) {
        for (const src of srcs) res.push([src, dst]);
      }
    }
...
function deriveBlocks(graph, s, root) {
  ...
  for (const [src, dests] of graph) {
    ...
    if (dests.length) {
      ...
      for (const curValue of opts) {
        ...
        if (value) {
          if (curScope.size > scope.size) continue;
      ...
      if (!value) throw new Error("INTERNAL: error on par");
      scope.add(src);
  ...
  for (const i of threads) {
    ...
    if (i.reff) {
      if (!effMemo.has(hash)) effMemo.set(hash, i);
      continue;
    }
    let threads = pureMemo.get(hash);
    if (!threads) pureMemo.set(hash, (threads = []));
    threads.push(i);
  }
  ...
  for (const i of threads) {
    const nleft = new Set();
    for (const j of i.left) nleft.add(j.replaceBy || j);
    i.left = [...nleft];
    for (const j of i.left) j.rc++;
  }
  /**
  ...
  for (const i of threads) {
    ...
    const op = node.value;
    if (op.origType !== Ctrl.jump) continue;
    if (op.frameArgs && op.frameArgs.size) continue;
    removedOps.push(op);
    i.removed = true;
  }
  threads = threads.filter(notRemoved);
  ...
  for (const i of removedOps) i.ref.removed = true;
  ...
  for (const i of threads) {
    ...
    if (toks.length || (lastExit && lastExit.sym && lastExit.sym.bound)) {
      ...
      if (errFrameRedir) errFrameRedir.required = true;
      if (resFrameRedir) resFrameRedir.required = true;
    } else {
      last.toks = false;
      last.exit = false;
  ...
  if (last.loop) {
    const {
      loopEnter: { frame: loopFrame }
    } = last;
    loopHead = s.tok(Tag.push, Block.frame, {
      declSym: Kit.scope.newSym("_plh"),
      savedDecls: new Map(),
    ...
    for (const [src] of graph) {
      if (src.block) {
        for (const i of src.value) chainFrames.add(i.frame);
      } else if (src.value.ref) chainFrames.add(src.value.ref);
    }
      ...
      if (cur.block) {
        if (cur.ret && cur.ret.jumps) {
          ...
          for (const k of cur.ret.jumps) {
            if (k.dir) setDirGoto(k.jump, goto);
            else if (k.indir) setIndirGoto(k.jump, goto);
          }
        }
      } else setGoto(cur.exit, next.frame);
    }
  }
  ...
  function getOpts(dests /*:LetVal[]*/) /*: Thread[][] */ {
    ...
    function _bt(pos, dests, cur) {
      ...
      if (dests.size !== next.size) _bt(pos + 1, next, [...cur, f]);
  ...
  function* emitFork(tup, joinFrame, tmpVar, parThread) {
    ...
    if (tmpVar) tmpVar.thread = parThread;
    if (joinFrame.dirEnters) joinFrame.dirEnters.add(res.value);
    yield res;
  ...
  function* emitFrames() {
    ...
    if (loopHead) {
      ...
      yield s.tok(Tag.push, Ctrl.jump, {
        ref: loopHead,
        goto: last.loopEnter.frame,
        reflected: true,
        tmpVar,
        bindName: forkJumpId.orig,
        opSym: forkJumpId
      });
    ...
    for (const i of threads) {
      ...
      if (expr.fork) {
        ...
        tmpVar.thread = i;
        yield jump;
        if (enter.dirEnters) enter.dirEnters.add(jump.value);
      }
      tmpVar.parLoop = false;
      yield* lab();
    }
    let chainCont = (defs.exit = yield* emitFork(tup, pureExitFrame));
    if (loopCont) chainCont = loopCont;
    else defs.joinGoto = chainCont;
    if (chainCont && exitGoto) {
      ...
      if (exitIndirGoto) {
        if (exitIndirGoto.dirEnters)
          (
            exitIndirGoto.indirEnters || (exitIndirGoto.indirEnters = new Set())
          ).add(chainCont);
      }
      if (exitGoto.dirEnters) exitGoto.dirEnters.add(chainCont);
    ...
    for (const i of threads) {
      for (const j of i.steps) {
        if (!j.block && j.toks && !j.removed) {
          ...
          for (const l of j.toks) yield* l.content;
          yield* s.leave();
        }
      }
    }
...
function adjustForkOps(si) {
  ...
  if (!root.hasPar || s.opts.inlineSharedOp) return s;
    ...
    for (const i of s) {
      if (i.enter) {
        switch (i.type) {
          case Block.letStmt:
          case Ctrl.jump:
            if (!i.value.goto) break;
            const { tmpVar } = i.value;
            const lab = s.label();
            if (!tmpVar || tmpVar.emptyThread) break;
            ...
            if (tmpVar.parLoop) tlsCopy = tmpVar.parLoop.tlsCopy;
            yield tlsCopy === null
            ...
            if (i.value.eff) {
              ...
              if (f.type === Block.app && f.value.sym === Block.pureId) {
                if (i.value.goto === root.pureExitFrame) {
                  ...
                  if (sub.length > 2) yield* sub.slice(1, sub.length - 1);
                }
                yield* s.leave();
                s.close(i);
              } else {
          ...
          case Block.app:
            if (i.value.sym === joinId) {
              ...
              for (const { tmpVar: j } of i.value.tup) {
                invariant(j);
                if (j.emptyThread) root.hasEmptyThreads = true;
                else vars.push(j);
              }
              if (vars.length === 0) {
...
export function calcCfg(frames) {
  ...
  while (jobs.length) {
    ...
    if (!job.start) {
      ...
      for (const i of node.nonRecExits) {
        if (!i.loops) continue;
        if (!loops) loops = node.loops = new Set();
        for (const j of i.loops) if (j !== node) loops.add(j);
      }
      if (node.tryWrap) jobs.push({ start: false, node: node.tryWrap });
      continue;
    }
    const { frame } = node;
    ...
    for (let i of frame.exits) {
      ...
      if (!goto || goto.dynamicJump) continue;
      ...
      if (fcc && jcc && jcc !== fcc && !fcc.handler) {
        ...
        for (let i = 0, len = js.length; i < len; ++i) {
          const hgoto = js[i];
          if (fs[i] !== hgoto) dest.unshift(hgoto);
        }
      ...
      if (enode.beg !== -1 && enode.end === -1) _recCon(node, enode);
      else _nonRecCon(node, enode);
    }
  }
  /**
  ...
  for (const i of nodes) {
    ...
    if (loops) {
      let loop;
      for (const j of loops) if (!loop || j.beg > loop.beg) loop = j;
  ...
  for (const i of nodes) {
    if (i.loop) {
      const stack = [];
      for (let j = i.loop; j; j = j.parLoop) stack.push(j);
  ...
  function _recCon(from, to) {
    if (from !== to) (from.loops || (from.loops = new Set())).add(to);
    from.exits.add(to);
    to.enters.add(from);
  ...
  function _tryNode(frame, handle) {
    return (
      handle.parCfgTryNode || (handle.parCfgTryNode = _makeNode(frame, true))
    );
  }
}
...
export function frameThreads(root, icfg) {
  ...
  if (icfg.length < 2) return icfchains;
  const first = icfg[0];
        ...
        for (const tasks = new Set(node.exits); tasks.size; ) {
          ...
          if (i.exits.size) {
            ...
            for (const i of residual) {
              if (i.handled || subLoopExits.has(i)) continue;
              if (subsetOf(i.enters, subBlock)) {
                residual.delete(i);
                i.available = true;
                if (!tasks.has(i)) tasks.add(i);
              }
            }
          } else {
            hasJoins = false;
          ...
          if (!tasks.size && subLoopExits.size) {
            for (const i of residual) {
              ...
              if (subBlock.has(i)) residual.delete(i);
              /** exiting to some outer block */ else if (
                nodeLoopExits &&
                nodeLoopExits.has(i)
              ) {
                residual.delete(i);
...
function splitByRegion(fchain) {
  for (const i of fchain) {
    if (i.block) {
      const value = [];
      const pseudo = [];
      for (const j of i.value) (j.tryBlock ? pseudo : value).push(j);
      i.value = value;
      i.pseudo = pseudo;
      i.first = i.value[0];
    } else {
    ...
    for (let j = chains.length - 1; j >= pos; --j) {
      ...
      if (chain) {
        ...
        for (const k of Kit.reverse(chain)) {
          ...
          } else skip.unshift(k.value);
        }
        chains[j] = null;
        if (chain.length > 1 && path[j].enabled) res.unshift(chain);
      }
    }
  }
  function _chainAt(pos) {
    let chain = chains[pos];
    if (!chain) chain = chains[pos] = [];
    return chain;
  }
}
    ...
    if (i.block) {
      ...
      for (const j of i.value) {
        for (const k of j.frame.exits) {
          const { goto } = k;
          if (goto.dynamicJump) continue;
          if (goto === join) jumps.push({ jump: k, dir: true, indir: false });
      ...
      for (const j of i.value.frame.steps) res.push({ block: false, value: j });
    }
  }
  return res;
}
...
function* substFrames(si) {
  ...
  let c;
  for (c of s) {
    if (c.leave && c.type === Block.chain) break;
    yield c;
  }
...
export function convert(si) {
  ...
  for (const i of frames) {
    for (const j of i.exits) {
      ...
      if (goto && !goto.dynamicJump) goto.dirEnters.add(j);
  ...
  root.parDefs = [];
  root.parFrames = [];
  for (const i of icfg) i._nodeVars = false;
  for (const nodes of jchains) nodes.forEach(nodeVars);
  for (const nodes of jchains) {
    const edges = edgesByVars(nodes);
    const graph = buildGraph(nodes, edges);
    if (deriveBlocks(graph, s, root)) root.hasPar = true;
  }
  if (!root.parDefs || !root.parDefs.length) return s;
...
function needsPar(jchain) {
  ...
  for (const i of jchain) {
    if (i.block || (i.value.eff && i.value.origType === Block.letStmt)) ++cnt;
...
export function markRegions(si) {
  ...
  if (!root.opts.par) return sa;
  let cnt = 0;
  _markRegions(
    [(root.rootParRegion = { id: ++cnt, enabled: root.opts.parRegion })],
    s
  );
  if (root.hasParRegions) root.opts.scopePrefix = true;
  return sa;
  function _markRegions(stack, sw) {
    const enabled = stack[stack.length - 1].enabled;
    if (enabled) root.hasParRegions = true;
    for (const i of sw) {
      if (i.enter && i.type === Block.chain) {
        i.value.parRegion = stack;
        const { parRegion } = i.value.opts;
        if (parRegion == null || parRegion === enabled) continue;
        _markRegions(
          (i.value.parRegion = [...stack, { id: ++cnt, enabled: parRegion }]),
          s.sub()
        );
      }
    }
...
function cleanEmptyThreads(si) {
  const s = Kit.auto(si);
  const root = s.first.value;
  if (!root.hasEmptyThreads) return s;
  return _cleanEmptyThreads();
  function* _cleanEmptyThreads() {
    for (const i of s) {
      if (
        i.enter &&
        i.type === Block.effExpr &&
        i.value.tmpVar &&
        i.value.tmpVar.emptyThread
      ) {
        Kit.skip(s.copy(i));
        continue;
      }
...
function contextThreading(si) {
  ...
  for (const fork of root.parDefs) {
          ...
          if (use.has(sym) && !confl.has(sym)) {
            ...
            if (info == null) {
              ...
              if (!i.loop) {
                ...
                mask.ownerFork = fork;
                if (loopLevel < 0) savedDecls.set(mask, { raw: null });
                else loopVars.add(mask);
              }
              forkConfls.set(
                sym,
                (info = {
    ...
    if (fork.loop) {
      ...
      for (const sym of rec.set) {
        ...
        if (plfork) plfork.loopVars.add(loopMask);
        else savedDecls.set(loopMask, {});
      }
    }
    ...
    for (const thread of fork.threads) {
      ...
      for (const i of thread.left) {
        for (const [sym, insts] of i.locals) {
          ...
          } else Kit.mapArr(locals, sym).push(...insts);
        }
      }
      for (const [sym, inst] of confl) locals.set(sym, [inst]);
    }
    /**
    ...
    for (const thread of fork.threads) {
      ...
      for (const [sym, inst] of confl) {
        if (inst.locPred.length > 1) {
          const mask = (inst.locMask = Kit.scope.newSym(
            `${sym.orig}$${inst.id}$`
          ));
          mask.ownerFork = fork;
          if (loopVars) loopVars.add(mask);
          else savedDecls.set(mask, { raw: null });
        }
        if (inst.copy) {
          inst.info.copies.add(inst);
          if (loopVars) loopVars.add(inst.copy);
          else savedDecls.set(inst.copy, { raw: null });
        }
      }
    }
    ...
    for (const thread of fork.threads) {
      ...
      if (pthread) {
        for (const [sym, pinst] of pthread.subst) {
          ...
          if (inst) inst.info.parent = pinst;
          else subst.set(sym, pinst);
        }
      }
    }
  }
  ...
  for (const fork of root.parDefs) {
    ...
    if (hasTls) {
      ...
      fork.tlsCopy = tlsSym;
    } else fork.tlsCopy = null;
  }
  for (const { loopVars } of root.parDefs)
    if (loopVars) State.allUniqFields(loopVars, "_");
  return _substVarThreads();
  function _copyVars(from, to) {
    for (const i of from) {
      if (i.local && !i.extBind) continue;
      to.add(i);
    }
  }
  function _contextTree(start, thread) {
    ...
    while (jobs.length) {
      ...
      if (frame.dynamicJump) {
        ...
        if (dst) jobs.push(dst);
        continue;
      }
      if (frame._contextTreeVisited) continue;
      frame._contextTreeVisited = true;
      frame.parThread =
        thread && thread.loop ? thread.fork.parentThread || false : thread;
      for (const i of frame.exits) {
        const fork = i.parJoin;
        if (fork && thread && !thread.loop) fork.parentThread = thread;
        const subThread = i.parThread;
        if (subThread) {
          _contextTree(i.goto, subThread);
        } else {
          if (i.goto) jobs.push(i.goto);
          if (i.indirJumps) {
            for (const [concr, abs] of i.indirJumps) vars.set(abs, concr);
          }
        }
      }
    }
      ...
      if (i.enter) {
        switch (i.type) {
          ...
          case Block.letStmt:
            ...
            if (!tmpVar) break;
            if (tmpVar.thread) {
              if (tmpVar.thread.fork.threads[0] !== tmpVar.thread) break;
      ...
      if (i.enter) {
        switch (i.type) {
          ...
          case Tag.AssignmentExpression:
            ...
            let buf;
            if (upd) {
              if (lhs.type !== Tag.Identifier && lhs.value.sym) break;
            ...
            yield* buf;
            if (!upd) yield* _substExpr(dict, fork, frame, s.one());
            yield* slab();
            s.close(i);
            for (const j of insts) yield* _assign(fork, j);
            if (!i.value.ignoreResult)
          ...
          case Block.frame:
            ...
            yield* _substJumps(
              thread && !thread.removed
                ? _substExpr(thread.subst, thread.fork, i.value, s.sub())
                : s.sub()
            );
            break;
        }
  ...
  function* _ref(pos, ctxFork, sym) {
    ...
    else yield* _tlsRef(pos, varFork, ctxFork, sym.fieldName);
  }
  ...
  function* _pref(pos, info, ctxFork) {
    if (info.parent) yield* _cref(pos, info.parent, ctxFork);
    else yield s.tok(pos, Tag.Identifier, { sym: info.sym });
  }
  ...
  function* _initLoop(fork, copy) {
    if (!fork.hasTls) return;
    const plfork = fork.parentLoopFork;
    const lab = s.label();
    if (!copy)
      for (const i of fork.loopMasks)
        yield* _assignRef(plfork, i, Tag.NumericLiteral, {
          node: { value: 0 }
        });
    yield s.enter(Tag.push, Tag.AssignmentExpression, {
      node: { operator: "=" }
    ...
    if (copy) yield* s.toks(Tag.value, `=$I.i+1`, tlsSym);
    else yield s.tok(Tag.value, Tag.NumericLiteral, { node: { value: 0 } });
    yield* s.leave();
    ...
    for (const [sym, info] of fork.confl) {
      ...
      else if (!copy) continue;
      for (const i of info.copies) {
        if (i.locMask)
          yield* _prop(i.locMask.fieldName, Tag.NumericLiteral, {
            node: { value: 0 }
          });
        yield s.enter(Tag.push, Tag.ObjectProperty);
  ...
  function* _init(fork) {
    for (const [sym, info] of fork.confl) {
      ...
      if (info.mask) yield* _ref(Tag.left, fork, info.mask);
      for (const i of info.copies) {
        if (!i.locMask) continue;
        yield s.enter(Tag.right, Tag.AssignmentExpression, {
          node: { operator: "=" }
        });
  ...
  function* _maskCond(ctxFork, mask, loopFork, value) {
    yield s.enter(Tag.push, Tag.LogicalExpression, {
      node: { operator: "||" }
    });
...
function setGoto(jump, newGoto) {
  if (jump.indirGoto) setIndirGoto(jump, newGoto);
  else setDirGoto(jump, newGoto);
}

function setDirGoto(jump, newGoto) {
  if (newGoto.dirEnters) newGoto.dirEnters.add(jump);
  if (jump.sym) newGoto.patSym = jump.sym;
  jump.goto = newGoto;
}

function setIndirGoto(jump, newGoto) {
  const oldGoto = jump.indirGoto;
  if (oldGoto === newGoto) return;
...
function subsetOf(u, s) {
  for (const i of u) if (!s.has(i)) return false;
  return true;
}
