...
export function inject(s) {
  ...
  function check(i) {
    ...
    return (
      qn != null && qn.length === 2 && qn[0] === s.opts.ns && qn[1] === "$"
    );
  }
  function* walk() {
...
export const split = Kit.pipe(
  function placeholderSplitMark(si) {
    ...
    function walk(sw) {
      for (const i of s.sub()) {
        if (i.enter && i.type === Tag.BlockStatement) block(i);
      }
    }
    walk();
    return sa;
          ...
          if (j.type === Tag.VariableDeclaration) {
            ...
            stmts = [
              ...(function*() {
                ...
                yield s.enter(Tag.push, Tag.ArrowFunctionExpression, {
                  node: { params: [] }
                });
