...
export function* repeat(num, tok) {
  for (let i = 0; i < num; i++) yield tok;
}

export function* completeAny(s) {
  const stack = [];
  for (const i of s) {
    if (i.enter) stack.unshift(i);
    if (i.leave) {
      const t = stack.shift();
      i.value = t.value;
      if (i.pos === Any) {
...
export function* sub(s) {
  const { lookahead } = s;
  if (lookahead.done) return;
  const { value } = lookahead;
  if (!value.enter) return;
  const exit = value.level;
  for (const i of s) {
    yield i;
    if (s.lookahead.done || s.lookahead.value.level < exit) return;
  }
}
...
export function* until(pred, s) {
  ...
  for (const i of s) {
    ...
    if (cur === exit || (cur === level && pred(c))) return;
  }
}
...
export function* tillLevel(level, s) {
  for (const i of s) {
    yield i;
    if (i.leave && i.level === level) return;
  }
}
...
export function* tillExit(s) {
  ...
  for (const i of s) {
    if (i.leave) {
      if (level == null || level > i.level) return;
    }
    yield i;
    if (i.enter) {
...
export function* findValue(pred, s) {
  for (const i of s) {
    yield i;
    if (i.enter && pred(i.value)) return i;
  }
  return null;
}
