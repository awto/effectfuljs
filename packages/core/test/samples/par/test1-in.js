...

{
  ...
  async function p14() {
    let j, k;
    const i = await init;
    var a1 = await k_1(await a_1(i++), (k += await m2)),
      a2 = await k_2(await a_2);
    j++;
    await g(a1, a2);
    await f(a1, j, k);
    await f2(a2, j, k);
  ...
  async function p17() {
    ...
    const w = await wery;
    let i = await init(w);
    var a1 = await k_1(await a_1(i++, w), (k += await m2(w)), w),
      a2 = await k_2(w, await a_2(w));
  ...
  lo: {
    try {
      ...
      try {
        try {
          await b_1;
          if (something) return await b_2;
          else if (somethingElse) break lo;
        } finally {
          await f_1;
        }
        await b_3;
...
async function r29() {
  ...
  {
    ("seq");
    if (somethingElse) console.log(([i] = "b"));
    await a_2_1;
  }
  {
    ("seq");
...
async function r33() {
  ...
  for (; something(i); ) {
    ...
    {
      ("seq");
      if (await somethingElse) console.log(([i] = "b"));
      await a_2_1;
    }
    {
      ("seq");
...
async function r46() {
  ...
  lo: for (;;) {
    for (;;) {
      if (something) continue lo;
      else if (sometingElse) break lo;
      await send(`lo`);
    }
  }
}
...
async function r47() {
  ...
  for (const i1 of [10, 20]) {
    for (const i2 of [1, 2]) {
      for (const i3 of ["A", "B"]) {
        let j = 0;
        if (await send(`sa:${i1}/${i2}/${i3}/${j}`, (30 - i1) / i2, t)) j = 10;
        if (await send(`sb:${i1}/${i2}/${i3}`, (30 - i1) / i2 + 2, t)) j = 20;
...
async function r50() {
  let i = 0;
  {
    ...
    {
      ("seq");
      if (await something_1) i = 1;
    }
    {
      ("seq");
      if (await something_2) i = 2;
    }
    {
      ("seq");
      await a_1(i);
